...
10. From inside the diorama, confirm pointing at worlds does not select or travel to them. Exit
    VR, then use the browser interface to select another destination.
11. Enter VR from a Solar System region and a black hole, and confirm each one offers the
