...
export const derivePlanetInferredProperties = (
  ...
  // Equilibrium temperature is a blackbody estimate for an airless body, not a surface
  // temperature, so a threshold set at "below freezing" is badly miscalibrated: Earth's is 255 K
  // and Mars's is 210 K, and the previous rule (ice above 0.6 for anything under 250 K) marked
  // both of them as ice worlds. Exposed surface ice only takes a body over once its equilibrium
  // temperature approaches the water frost line — Europa 102 K, Enceladus 75 K, Pluto 44 K —
  // while roughly 170-240 K is the cold-desert regime Mars actually occupies: polar caps and
  // seasonal frost over bare, dust-mantled rock.
  ...
  // Standing liquid water needs more than "not scorched". Below roughly 240 K a bare surface is
  // frozen the year round whatever its volatile budget, and much above 340 K it is boiling off,
  // so ocean likelihood is confined to that band rather than to the whole 180-330 K range the
  // previous rule called temperate — which is what handed Mars-temperature worlds an ocean.
  const isLiquidWaterBand = temperature !== null && temperature >= 240 && temperature <= 340;
  // Within the band, a bulk density at or below bare silicate (~3 g/cm^3) reads as volatile-rich
  // and an iron-rich one (~8) as desiccated. Earth's own 5.51 sits mid-scale: its ocean is 0.02%
  // of its mass and is invisible to bulk density, so this can neither confirm nor exclude one.
...
/** Mineral palettes are keyed by the same chemistry family inferred from density and thermal
 * regime. Previously that family was recorded in the recipe but most rocky worlds were painted
 * by one temperature-only brown palette, which erased much of the catalog's visual variety. */
...
const deriveRockyRecipe = (
  ...
  /**
   * Thermal regimes, keyed to where the volatiles actually go rather than to one broad
   * "temperate" band. The Solar System supplies the calibration points at every boundary:
   * Venus 232 K equilibrium but scorched by its own greenhouse, Earth 255 K with standing water,
   * Mars 210 K as a cold desert of bare rock under polar caps, Europa 102 K glaciated edge to
   * edge. The band that used to run 180-330 K swept Mars in with Earth and gave it an ocean.
   */
  const isScorched = temperature !== null && temperature > 500;
  /** Hot, dry rock: above the water boiling regime, below the point of melting the surface. */
  ...
  /** Mars's regime: frost migrates to the poles seasonally and low latitudes stay bare rock. */
  ...
  // Preserve the inferred mineral chemistry through thermal transformations. A frozen sulfur
  // world should still carry sulfur beneath its frost, and two hot rocky worlds should not both
  // collapse to the exact same orange lava palette.
  const palette = {
    lowColor: scaleColor(
      mixColor(
  ...
  // Standing water follows the inference rather than the thermal band alone: a world can sit in
  // the liquid-water band and still be a dry, iron-rich rock, which is what `oceanLikelihood`
  // reads off bulk density. A cold desert gets none at all — its water is locked in the caps.
  ...
  const iceCapStrength = isGlobalIce
    ...
    : isFrozen
      ? 0.78 + random() * 0.18
      : isColdDesert
        ? // Polar caps and a frost-streaked mid-latitude, not a glaciated world: Mars carries
          // permanent ice over roughly a hundredth of its surface.
          0.1 + inferred.iceLikelihood * 0.24
        : isTemperate
          ? 0.18 + random() * 0.22
          : 0;
  // Cloud cover needs water in the air, which needs water on the ground and an atmosphere to
  // hold it. A cold desert is close to airless in this respect — Mars runs a few percent.
  ...
  return {
    ...
    terrain: {
      ...
      // Running water and thick air round terrain off; a cold desert erodes by wind alone, which
      // polishes and buries without cutting valley networks, so it lands between the two.
      ...
      polarIceBias: isGlobalIce
        ...
        : isFrozen
          ? 0.55 + random() * 0.4
          : isColdDesert
            ? // Nearly all of a cold desert's exposed ice sits at the poles.
              0.7 + random() * 0.25
            : isTemperate
              ? 0.15 + random() * 0.2
              : 0,
