...
export const derivePlanetInferredProperties = (
  ...
  const iceLikelihood = temperature === null ? 0.35 : clampUnit((240 - temperature) / 110);
  ...
  const isLiquidWaterBand = temperature !== null && temperature >= 240 && temperature <= 340;
  const oceanLikelihood = !isLiquidWaterBand
    ? 0
    : density !== null
      ? clampUnit(1 - (density - 3) / 4)
      : 0.35;
...
const ROCKY_SURFACE_PALETTES: Record<RockyPaletteFamily, RockySurfacePalette> = {
  "basaltic-dark": {
    lowColor: [0.018, 0.024, 0.032],
    midColor: [0.11, 0.135, 0.15],
    highColor: [0.34, 0.37, 0.37],
    waterColor: [0.008, 0.04, 0.055],
    atmosphere: [0.32, 0.44, 0.58],
  },
  "carbon-dark": {
    lowColor: [0.008, 0.01, 0.016],
    midColor: [0.055, 0.07, 0.095],
    highColor: [0.22, 0.25, 0.3],
    waterColor: [0.006, 0.018, 0.03],
    atmosphere: [0.3, 0.38, 0.58],
  },
  "desert-tan": {
    lowColor: [0.15, 0.065, 0.025],
    midColor: [0.53, 0.29, 0.095],
    highColor: [0.9, 0.7, 0.38],
    waterColor: [0.04, 0.11, 0.12],
    atmosphere: [0.72, 0.5, 0.28],
  },
  "ice-blue": {
    lowColor: [0.035, 0.085, 0.14],
    midColor: [0.31, 0.48, 0.61],
    highColor: [0.86, 0.95, 0.98],
    waterColor: [0.012, 0.075, 0.14],
    atmosphere: [0.34, 0.7, 1],
  },
  "iron-rich": {
    lowColor: [0.025, 0.035, 0.04],
    midColor: [0.18, 0.21, 0.2],
    highColor: [0.5, 0.53, 0.48],
    waterColor: [0.008, 0.055, 0.052],
    atmosphere: [0.38, 0.46, 0.5],
  },
  "lava-black-red": {
    lowColor: [0.012, 0.008, 0.012],
    midColor: [0.15, 0.025, 0.018],
    highColor: [0.58, 0.13, 0.035],
    waterColor: [0.09, 0.012, 0.008],
    atmosphere: [0.95, 0.2, 0.06],
  },
  "oxidized-red": {
    lowColor: [0.095, 0.025, 0.018],
    midColor: [0.42, 0.105, 0.045],
    highColor: [0.78, 0.38, 0.16],
    waterColor: [0.075, 0.025, 0.015],
    atmosphere: [0.68, 0.32, 0.19],
  },
  "silicate-neutral": {
    lowColor: [0.07, 0.065, 0.06],
    midColor: [0.34, 0.3, 0.25],
    highColor: [0.7, 0.67, 0.59],
    waterColor: [0.012, 0.12, 0.23],
    atmosphere: [0.24, 0.62, 0.96],
  },
  "sulfuric-yellow": {
    lowColor: [0.12, 0.075, 0.008],
    midColor: [0.55, 0.39, 0.045],
    highColor: [0.94, 0.77, 0.22],
    waterColor: [0.11, 0.13, 0.018],
    atmosphere: [0.82, 0.62, 0.16],
  },
};
...
const deriveGasGiantRecipe = (
  ...
  const paletteTone = 0.9 + random() * 0.2;
  const paletteLift = random() * 0.12;
  const deepColor = scaleColor(palette.deepColor, paletteTone * (0.9 + random() * 0.12));
  const midColor = mixColor(
    scaleColor(palette.midColor, paletteTone),
    palette.lightColor,
    paletteLift,
  );
  const lightColor = mixColor(
    scaleColor(palette.lightColor, 0.94 + random() * 0.12),
    palette.midColor,
    random() * 0.07,
  );
...
const deriveRockyRecipe = (
  ...
  const isHotDesert = temperature !== null && temperature > 340 && temperature <= 500;
  const isTemperate = temperature !== null && temperature >= 240 && temperature <= 340;
  const isColdDesert = temperature !== null && temperature >= 170 && temperature < 240;
  const isFrozen = temperature !== null && temperature < 170;
  ...
  const mineralPalette = ROCKY_SURFACE_PALETTES[paletteFamily];
  const frozenBlend = isGlobalIce ? 0.78 : isFrozen ? 0.5 : 0;
  const scorchedBlend = isScorched ? clampUnit(0.35 + inferred.volcanicLikelihood * 0.45) : 0;
  const seedTone = 0.9 + random() * 0.18;

  const palette = {
    lowColor: scaleColor(
      mixColor(
        mixColor(mineralPalette.lowColor, [0.035, 0.075, 0.12], frozenBlend),
        [0.012, 0.006, 0.008],
        scorchedBlend,
      ),
      seedTone,
    ...
    midColor: scaleColor(
      mixColor(
        mixColor(mineralPalette.midColor, [0.34, 0.5, 0.62], frozenBlend),
        [0.42, 0.055, 0.018],
        scorchedBlend,
      ),
      seedTone,
    ),
    highColor: scaleColor(
      mixColor(
        mixColor(mineralPalette.highColor, [0.88, 0.96, 0.99], frozenBlend),
        [0.96, 0.34, 0.065],
        scorchedBlend * 0.72,
      ),
      seedTone,
    ...
    waterColor: isFrozenOcean
      ? mixColor(mineralPalette.waterColor, [0.78, 0.91, 0.97], 0.58)
      : mineralPalette.waterColor,
    atmosphere: mixColor(
      mixColor(mineralPalette.atmosphere, [0.34, 0.72, 1], frozenBlend * 0.72),
      [1, 0.18, 0.045],
      scorchedBlend * 0.7,
    ),
  };
  ...
  const waterLevel = isTemperate
    ? clampUnit(0.12 + inferred.oceanLikelihood * 0.45)
    : isFrozenOcean
      ? 0.36 + random() * 0.1
      : 0;
  ...
  const iceCapStrength = isGlobalIce
    ...
    : isFrozen
      ? 0.78 + random() * 0.18
      : isColdDesert
        ? 0.1 + inferred.iceLikelihood * 0.24
        : isTemperate
          ? 0.18 + random() * 0.22
          : 0;
  const cloudCover = isTemperate
    ? clampUnit(0.16 + waterLevel * 0.7 + random() * 0.16)
    : isColdDesert
      ? 0.02 + random() * 0.08
      : isFrozen
        ? 0.08 + random() * 0.14
...
const deriveIceGiantRecipe = (
  ...
  const paletteTone = 0.9 + random() * 0.2;
  const deepColor = scaleColor(palette.deepColor, paletteTone * (0.9 + random() * 0.12));
  const hazeColor = mixColor(
    scaleColor(palette.hazeColor, paletteTone),
    palette.lightColor,
    random() * 0.1,
  );
  const lightColor = scaleColor(palette.lightColor, 0.94 + random() * 0.12);
